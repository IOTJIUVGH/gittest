...
//#define device_O2_log(M, ...)				custom_log("", M, ##__VA_ARGS__)
#define device_O2_log(M, ...)

// O2
enum _Air_properties_property_no
{
	APPN_OpMode									= 0x01,
	APPN_Power									= 0x02,
	APPN_ChildLock								= 0x03,
	...
	APPN_AQIt									= 0x09,
	APPN_TVOC									= 0x0A,
	APPN_DeviceDisplayParameter					= 0x0B,
	APPN_RealtimeDeviceDisplayParameter			= 0x0C,
	APPN_ErrorCode								= 0x0D,
};
...
void philips_encode_json_O2(char **buf, const char *StatusType, const char *ConnectType, bool head)
{
	...
	cJSON_AddStringToObject(item, "range", "O2");
	...
	{
		cJSON_AddNumberToObject(item, "fltsts0", get_philips_running_status()->o2_filter_flash_param.filter0_life);
		cJSON_AddNumberToObject(item, "fltsts1", get_philips_running_status()->o2_filter_flash_param.filter1_life);
	}
...
void philips_process_air_properties_O2(const uint8_t *data)
{
    ...
    while (*(p) != 0x00)
    {
		...
		case APPN_OpMode:
		{
			...
			case 0x05: str_value[0] = 'a'; str_value[1] = '\0'; break;
			default: str_value[0] = '\0'; break;
			}
			if ( str_value[0] == '\0' )
				break;
			device_O2_log("APPN_FanSpeed: value = %s, purifier_status.om = %s", str_value, purifier_status.om_str);
		...
		case APPN_Power:
		{
			...
			device_O2_log("APPN_Power: value = %s, purifier_status.pwr = %s", str_value, purifier_status.pwr_str);
		...
		case APPN_ChildLock:
		{
			...
			device_O2_log("APPN_ChildLock: value = %d, purifier_status.cl = %d", b_value, purifier_status.cl);
		...
		{
			itoa(*(p + 2), str_value, 10);
			device_O2_log("APPN_AQILight: value = %s, purifier_status.aqil = %s", str_value, purifier_status.aqil_str);
		...
		{
			itoa(*(p + 2), str_value, 10);
			device_O2_log("APPN_UILight: value = %s, purifier_status.uil = %s", str_value, purifier_status.uil_str);
		...
		case APPN_Mode:
		{
			...
			case 0x02: str_value[0] = 'P'; str_value[1] = '\0'; str_value[2] = '\0'; break;
			case 0x03: str_value[0] = 'F'; str_value[1] = '\0'; str_value[2] = '\0'; break;
			case 0x04: str_value[0] = 'A'; str_value[1] = '\0'; str_value[2] = '\0'; break;
			default: str_value[0] = '\0'; break;
			}
			if ( str_value[0] == '\0' )
				break;
			device_O2_log("APPN_Mode: value = %s, purifier_status.mode = %s", str_value, purifier_status.mode_str);
		...
		case APPN_DeviceDisplayParameter:
		{
			purifier_status.air_prop_map |= PHILIPS_AIR_PROP_MAP_DDP;
			itoa(*(p + 2), str_value, 10);
			device_O2_log("APPN_DeviceDisplayParameter: value = %s, purifier_status.ddp = %s", str_value, purifier_status.ddp_str);
		...
		case APPN_RealtimeDeviceDisplayParameter:
		{
			purifier_status.air_prop_map |= PHILIPS_AIR_PROP_MAP_RDDP;
			itoa(*(p + 2), str_value, 10);
			device_O2_log("APPN_RealtimeDeviceDisplayParameter: value = %s, purifier_status.rddp = %s", str_value, purifier_status.rddp_str);
		...
		case APPN_ErrorCode:
		{
			...
			device_O2_log("purifier_status.err = %d, deviceerrorcode = %lu, warn = 0x%08X, error = 0x%08X", purifier_status.err, deviceerrorcode, get_philips_running_status()->device_upload_manage.warn, get_philips_running_status()->device_upload_manage.error);
			device_O2_log("...");
