...
//#define device_Comfort_log(M, ...)				custom_log("", M, ##__VA_ARGS__)
#define device_Comfort_log(M, ...)

//Comfort
enum _Air_properties_property_no_Comfort
{
	APPN_Comfort_FanSpeed								= 0x01,
	APPN_Comfort_Power									= 0x02,
	APPN_Comfort_ChildLock								= 0x03,
	APPN_Comfort_AQILight								= 0x04,
	APPN_Comfort_UILight								= 0x05,
	APPN_Comfort_DeviceTimer							= 0x06,
	APPN_Comfort_DeviceTimerRemaining					= 0x07,
	APPN_Comfort_OperationMode							= 0x08,
	APPN_Comfort_PM2_5									= 0x09,
	APPN_Comfort_iAQL									= 0x0A,
	APPN_Comfort_AQIt									= 0x0B,
	APPN_Comfort_DeviceDisplayParameter					= 0x0C,
	APPN_Comfort_ErrorCode								= 0x0D,
};
...
void philips_encode_json_Comfort(char **buf, const char *StatusType, const char *ConnectType, bool head)
{
	...
	cJSON_AddStringToObject(item, "range", "Comfort");
...
void philips_process_air_properties_Comfort(const uint8_t *data)
{
    ...
    while (*(p) != 0x00)
    {
		...
		case APPN_Comfort_FanSpeed:
		{
			...
			if ( str_value[0] == '\0' )
				break;
			device_Comfort_log("APPN_FanSpeed: value = %s, purifier_status.om = %s", str_value, purifier_status.om_str);
		...
		case APPN_Comfort_Power:
		{
			itoa(*(p + 2), str_value, 10);
			device_Comfort_log("APPN_Power: value = %s, purifier_status.pwr = %s", str_value, purifier_status.pwr_str);
		...
		case APPN_Comfort_ChildLock:
		{
			b_value = (*(p + 2) == 1) ? true : false;
			device_Comfort_log("APPN_ChildLock: value = %d, purifier_status.cl = %d", b_value, purifier_status.cl);
		...
		case APPN_Comfort_AQILight:
		{
			i_value = *(p + 2);
			device_Comfort_log("APPN_AQILight: value = %d, purifier_status.aqil = %d", i_value, purifier_status.aqil);
		...
		case APPN_Comfort_UILight:
		{
			...
			if( str_value[0] == '\0' )
				break;
			device_Comfort_log("APPN_UILight: value = %s, purifier_status.uil = %s", str_value, purifier_status.uil_str);
		...
		case APPN_Comfort_DeviceTimer:
		{
			i_value = *(p + 2);
			device_Comfort_log("APPN_DeviceTimer: value = %d, purifier_status.dt = %d", i_value, purifier_status.dt);
		...
		case APPN_Comfort_DeviceTimerRemaining:
		{
			i_value = (*(p + 2) << 8) + *(p + 3);
			device_Comfort_log("APPN_DeviceTimerRemaining: value = %d, purifier_status.dtrs = %d", i_value, purifier_status.dtrs);
		...
		case APPN_Comfort_OperationMode:
		{
			switch ( *(p + 2) )
			{
			...
			case 0x04: str_value[0] = 'H'; str_value[1] = '\0'; break;
			default: str_value[0] = '\0'; break;
			}
			if ( str_value[0] == '\0' )
				break;
			device_Comfort_log("APPN_Mode: value = %s, purifier_status.mode = %s", str_value, purifier_status.mode_str);
		...
		case APPN_Comfort_AQIt:
		{
			purifier_status.air_prop_map |= PHILIPS_AIR_PROP_MAP_AQIT;
			break;
		}
		case APPN_Comfort_DeviceDisplayParameter:
		{
			purifier_status.air_prop_map |= PHILIPS_AIR_PROP_MAP_DDP;
			itoa(*(p + 2), str_value, 10);
			device_Comfort_log("APPN_DeviceDisplayParameter: value = %s, purifier_status.ddp = %s", str_value, purifier_status.ddp_str);
		...
		case APPN_Comfort_ErrorCode:
		{
			...
			else if ( (deviceerrorcode >> 7) & 0x01 )	// error
			{
				...
				case 0x83: get_philips_running_status()->device_upload_manage.error |= PHILIPS_ERROR_E4; break;	// Gas sensor error
				default: break;
				}
			}

			device_Comfort_log("purifier_status.err = %d, deviceerrorcode = %lu, warn = 0x%08X, error = 0x%08X", purifier_status.err, deviceerrorcode, get_philips_running_status()->device_upload_manage.warn, get_philips_running_status()->device_upload_manage.error);
			device_Comfort_log("...");
...
void philips_process_cloud_data_Comfort(char *data, int datalen)
{
	...
	if(item)
	{
		...
		else if(strcmp(item->valuestring, "H") == 0)
		{
			value_8 = 0x04;
		}
		else
