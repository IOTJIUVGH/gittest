...
//#define device_PUMA_log(M, ...)				custom_log("", M, ##__VA_ARGS__)
#define device_PUMA_log(M, ...)

// PUMA
enum _Air_properties_property_no_PUMA
{
	APPN_PUMA_FanSpeed										= 0x01,
	APPN_PUMA_Power											= 0x02,
	APPN_PUMA_ChildLock										= 0x03,
	APPN_PUMA_AQILight										= 0x04,
	APPN_PUMA_UILight										= 0x05,
	APPN_PUMA_DeviceTimer									= 0x06,
	APPN_PUMA_DeviceTimerRemaining							= 0x07,
	APPN_PUMA_OperationMode									= 0x08,
	APPN_PUMA_PM2_5											= 0x09,
	APPN_PUMA_iAQL											= 0x0A,
	APPN_PUMA_AQIt											= 0x0B,
	APPN_PUMA_DeviceDisplayParameter						= 0x0C,
	APPN_PUMA_ErrorCode										= 0x0D,
};
...
void philips_encode_json_PUMA(char **buf, const char *StatusType, const char *ConnectType, bool head)
{
	...
	cJSON_AddStringToObject(item, "range", "PUMA");
...
void philips_process_air_properties_PUMA(const uint8_t *data)
{
    ...
    while (*(p) != 0x00)
    {
		...
		case APPN_PUMA_FanSpeed:
		{
			...
			if ( str_value[0] == '\0' )
				break;
			device_PUMA_log("APPN_FanSpeed: value = %s, purifier_status.om = %s", str_value, purifier_status.om_str);
		...
		case APPN_PUMA_Power:
		{
			itoa(*(p + 2), str_value, 10);
			device_PUMA_log("APPN_Power: value = %s, purifier_status.pwr = %s", str_value, purifier_status.pwr_str);
		...
		case APPN_PUMA_ChildLock:
		{
			b_value = (*(p + 2) == 1) ? true : false;
			device_PUMA_log("APPN_ChildLock: value = %d, purifier_status.cl = %d", b_value, purifier_status.cl);
		...
		case APPN_PUMA_AQILight:
		{
			i_value = *(p + 2);
			device_PUMA_log("APPN_AQILight: value = %d, purifier_status.aqil = %d", i_value, purifier_status.aqil);
		...
		case APPN_PUMA_UILight:
		{
			...
			if ( str_value[0] == '\0' )
				break;
			device_PUMA_log("APPN_UILight: value = %s, purifier_status.uil = %s", str_value, purifier_status.uil_str);
		...
		case APPN_PUMA_DeviceTimer:
		{
			i_value = *(p + 2);
			device_PUMA_log("APPN_DeviceTimer: value = %d, purifier_status.dt = %d", i_value, purifier_status.dt);
		...
		case APPN_PUMA_DeviceTimerRemaining:
		{
			i_value = (*(p + 2) << 8) + *(p + 3);
			device_PUMA_log("APPN_DeviceTimerRemaining: value = %d, purifier_status.dtrs = %d", i_value, purifier_status.dtrs);
		...
		case APPN_PUMA_OperationMode:
		{
			...
			if ( str_value[0] == '\0' )
				break;
			device_PUMA_log("APPN_Mode: value = %s, purifier_status.mode = %s", str_value, purifier_status.mode_str);
		...
		case APPN_PUMA_DeviceDisplayParameter:
		{
			purifier_status.air_prop_map |= PHILIPS_AIR_PROP_MAP_DDP;
			itoa(*(p + 2), str_value, 10);
			device_PUMA_log("APPN_DeviceDisplayParameter: value = %s, purifier_status.ddp = %s", str_value, purifier_status.ddp_str);
		...
		case APPN_PUMA_ErrorCode:
		{
			...
			device_PUMA_log("purifier_status.err = %d, deviceerrorcode = %lu, warn = 0x%08X, error = 0x%08X", purifier_status.err, deviceerrorcode, get_philips_running_status()->device_upload_manage.warn, get_philips_running_status()->device_upload_manage.error);
			device_PUMA_log("...");
