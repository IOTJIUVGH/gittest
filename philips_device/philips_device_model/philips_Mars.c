...
//#define device_Mars_log(M, ...)				custom_log("", M, ##__VA_ARGS__)
#define device_Mars_log(M, ...)

// Mars
enum _Air_properties_property_no_Mars
{
	APPN_Mars_FanSpeed								= 0x01,
	APPN_Mars_Power									= 0x02,
	APPN_Mars_ChildLock								= 0x03,
	APPN_Mars_AQILight								= 0x04,
	APPN_Mars_UILight								= 0x05,
	APPN_Mars_UserAutoModeSetting					= 0x06,
	APPN_Mars_OperationMode							= 0x07,
	APPN_Mars_PM2_5									= 0x08,
	APPN_Mars_iAQL									= 0x09,
	APPN_Mars_TVOC									= 0x0A,
	APPN_Mars_DeviceDisplayParameter				= 0x0B,
	APPN_Mars_RealtimeDeviceDisplayParameter		= 0x0C,
	APPN_Mars_ErrorCode								= 0x0D,
	APPN_Mars_AQIt									= 0x0E,
};
...
void philips_process_air_properties_Mars(const uint8_t *data)
{
    ...
    while (*(p) != 0x00)
    {
		...
		case APPN_Mars_FanSpeed:
		{
			...
			if ( str_value[0] == '\0' )
				break;
			device_Mars_log("APPN_FanSpeed: value = %s, purifier_status.om = %s", str_value, purifier_status.om_str);
		...
		case APPN_Mars_Power:
		{
			purifier_status.air_prop_map |= PHILIPS_AIR_PROP_MAP_PWR;
			itoa(*(p + 2), str_value, 10);
			device_Mars_log("APPN_Power: value = %s, purifier_status.pwr = %s", str_value, purifier_status.pwr_str);
		...
		case APPN_Mars_ChildLock:
		{
			purifier_status.air_prop_map |= PHILIPS_AIR_PROP_MAP_CL;
			b_value = (*(p + 2) == 1) ? true : false;
			device_Mars_log("APPN_ChildLock: value = %d, purifier_status.cl = %d", b_value, purifier_status.cl);
		...
		case APPN_Mars_AQILight:
		{
			purifier_status.air_prop_map |= PHILIPS_AIR_PROP_MAP_AQIL;
			i_value = *(p + 2);
			device_Mars_log("APPN_AQILight: value = %d, purifier_status.aqil = %d", i_value, purifier_status.aqil);
		...
		case APPN_Mars_UILight:
		{
			...
			if ( str_value[0] == '\0' )
				break;
			device_Mars_log("APPN_UILight: value = %s, purifier_status.uil = %s", str_value, purifier_status.uil_str);
		...
		case APPN_Mars_UserAutoModeSetting:
		{
			...
			if ( str_value[0] == '\0' )
				break;
			device_Mars_log("APPN_Mode: value = %s, purifier_status.uaset = %s", str_value, purifier_status.uaset_str);
		...
		case APPN_Mars_OperationMode:
		{
			...
			if ( str_value[0] == '\0' )
				break;
			device_Mars_log("APPN_Mode: value = %s, purifier_status.mode = %s", str_value, purifier_status.mode_str);
		...
		case APPN_Mars_DeviceDisplayParameter:
		{
			purifier_status.air_prop_map |= PHILIPS_AIR_PROP_MAP_DDP;
			itoa(*(p + 2), str_value, 10);
			device_Mars_log("APPN_DeviceDisplayParameter: value = %s, purifier_status.ddp = %s", str_value, purifier_status.ddp_str);
		...
		case APPN_Mars_RealtimeDeviceDisplayParameter:
		{
			purifier_status.air_prop_map |= PHILIPS_AIR_PROP_MAP_RDDP;
			itoa(*(p + 2), str_value, 10);
			device_Mars_log("APPN_RealtimeDeviceDisplayParameter: value = %s, purifier_status.rddp = %s", str_value, purifier_status.rddp_str);
...
//						case 5: get_philips_running_status()->device_upload_manage.warn |= (1 << 11); break;	// Mars Filter 2 lock
			...
			device_Mars_log("purifier_status.err = %d, deviceerrorcode = %lu, warn = 0x%08X, error = 0x%08X", purifier_status.err, deviceerrorcode, get_philips_running_status()->device_upload_manage.warn, get_philips_running_status()->device_upload_manage.error);
			device_Mars_log("...");
