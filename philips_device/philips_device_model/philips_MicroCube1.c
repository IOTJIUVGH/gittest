...
//#define device_MicroCube1_log(M, ...)				custom_log("", M, ##__VA_ARGS__)
#define device_MicroCube1_log(M, ...)

// MicroCube1
enum _Air_properties_property_no_MicroCube1
{
	APPN_MicroCube1_OpMode									= 0x01,
	APPN_MicroCube1_Power									= 0x02,
	APPN_MicroCube1_ChildLock								= 0x03,
	APPN_MicroCube1_AQILight								= 0x04,
	APPN_MicroCube1_UILight									= 0x05,
	APPN_MicroCube1_DeviceTimer								= 0x06,
	APPN_MicroCube1_DeviceTimerRemaining					= 0x07,
	APPN_MicroCube1_Mode									= 0x08,
	APPN_MicroCube1_PM2_5									= 0x09,
	APPN_MicroCube1_iAQL									= 0x0A,
	APPN_MicroCube1_AQIt									= 0x0B,
	APPN_MicroCube1_TVOC									= 0x0C,
	APPN_MicroCube1_DeviceDisplayParameter					= 0x0D,
	APPN_MicroCube1_RealtimeDeviceDisplayParameter			= 0x0E,
	APPN_MicroCube1_ErrorCode								= 0x0F,
};
...
void philips_encode_json_MicroCube1(char **buf, const char *StatusType, const char *ConnectType, bool head)
{
	...
	cJSON_AddStringToObject(item, "range", "MicroCube1.0");
...
void philips_process_air_properties_MicroCube1(const uint8_t *data)
{
    ...
    while (*(p) != 0x00)
    {
		...
		case APPN_MicroCube1_OpMode:
		{
			...
			if ( str_value[0] == '\0' )
				break;
			device_MicroCube1_log("APPN_FanSpeed: value = %s, purifier_status.om = %s", str_value, purifier_status.om_str);
		...
		case APPN_MicroCube1_Power:
		{
			itoa(*(p + 2), str_value, 10);
			device_MicroCube1_log("APPN_Power: value = %s, purifier_status.pwr = %s", str_value, purifier_status.pwr_str);
		...
		case APPN_MicroCube1_ChildLock:
		{
			b_value = (*(p + 2) == 1) ? true : false;
			device_MicroCube1_log("APPN_ChildLock: value = %d, purifier_status.cl = %d", b_value, purifier_status.cl);
		...
		case APPN_MicroCube1_AQILight:
		{
			i_value = *(p + 2);
			device_MicroCube1_log("APPN_AQILight: value = %d, purifier_status.aqil = %d", i_value, purifier_status.aqil);
		...
		case APPN_MicroCube1_UILight:
		{
			itoa(*(p + 2), str_value, 10);
			device_MicroCube1_log("APPN_UILight: value = %s, purifier_status.uil = %s", str_value, purifier_status.uil_str);
		...
		case APPN_MicroCube1_DeviceTimer:
		{
			i_value = *(p + 2);
			device_MicroCube1_log("APPN_DeviceTimer: value = %d, purifier_status.dt = %d", i_value, purifier_status.dt);
		...
		case APPN_MicroCube1_DeviceTimerRemaining:
		{
			i_value = (*(p + 2) << 8) + *(p + 3);
			device_MicroCube1_log("APPN_DeviceTimerRemaining: value = %d, purifier_status.dtrs = %d", i_value, purifier_status.dtrs);
		...
		case APPN_MicroCube1_Mode:
		{
			...
			if ( str_value[0] == '\0' )
				break;
			device_MicroCube1_log("APPN_Mode: value = %s, purifier_status.mode = %s", str_value, purifier_status.mode_str);
		...
		case APPN_MicroCube1_DeviceDisplayParameter:
		{
			purifier_status.air_prop_map |= PHILIPS_AIR_PROP_MAP_DDP;
			itoa(*(p + 2), str_value, 10);
			device_MicroCube1_log("APPN_DeviceDisplayParameter: value = %s, purifier_status.ddp = %s", str_value, purifier_status.ddp_str);
		...
		case APPN_MicroCube1_RealtimeDeviceDisplayParameter:
		{
			purifier_status.air_prop_map |= PHILIPS_AIR_PROP_MAP_RDDP;
			itoa(*(p + 2), str_value, 10);
			device_MicroCube1_log("APPN_RealtimeDeviceDisplayParameter: value = %s, purifier_status.rddp = %s", str_value, purifier_status.rddp_str);
		...
		case APPN_MicroCube1_ErrorCode:
		{
			...
			else if ( (deviceerrorcode >> 7) & 0x01 )	// error
			{
				...
				case 0x84: get_philips_running_status()->device_upload_manage.error |= PHILIPS_ERROR_E4; break;	// Gas sensor error
				default: break;
				}
			}

			device_MicroCube1_log("purifier_status.err = %d, deviceerrorcode = %lu, warn = 0x%08X, error = 0x%08X", purifier_status.err, deviceerrorcode, get_philips_running_status()->device_upload_manage.warn, get_philips_running_status()->device_upload_manage.error);
			device_MicroCube1_log("...");
