...
//#define device_MicroMario_log(M, ...)				custom_log("", M, ##__VA_ARGS__)
#define device_MicroMario_log(M, ...)

// MicroMario
enum _Air_properties_property_no_MicroMario
{
	APPN_MicroMario_FanSpeed					= 0x01,
	APPN_MicroMario_Power						= 0x02,
	APPN_MicroMario_ChildLock					= 0x03,
	APPN_MicroMario_AQILight					= 0x04,
	APPN_MicroMario_UILight						= 0x05,
	APPN_MicroMario_DeviceTimer					= 0x06,
	APPN_MicroMario_DeviceTimerRemaining		= 0x07,
	APPN_MicroMario_OperationMode				= 0x08,
	APPN_MicroMario_Function					= 0x09,
	APPN_MicroMario_RHSetting					= 0x0A,
	APPN_MicroMario_RH							= 0x0B,
	APPN_MicroMario_Temperature					= 0x0C,
	APPN_MicroMario_PM2_5						= 0x0D,
	APPN_MicroMario_iAQL						= 0x0E,
	APPN_MicroMario_AQIt						= 0x0F,
	APPN_MicroMario_DeviceDisplayParameter		= 0x10,
	APPN_MicroMario_RealtimeDeviceDisplayParameter			= 0x11,
	APPN_MicroMario_ErrorCode								= 0x12,
	APPN_MicroMario_WaterLevel								= 0x13,
};
...
void philips_encode_json_MicroMario(char **buf, const char *StatusType, const char *ConnectType, bool head)
{
	...
	cJSON_AddStringToObject(item, "range", "MicroMario");
...
void philips_process_air_properties_MicroMario(const uint8_t *data)
{
    ...
    while (*(p) != 0x00)
    {
		...
		case APPN_MicroMario_FanSpeed:
		{
			...
			if ( str_value[0] == '\0' )
				break;
			device_MicroMario_log("APPN_FanSpeed: value = %s, purifier_status.om = %s", str_value, purifier_status.om_str);
		...
		case APPN_MicroMario_Power:
		{
			itoa(*(p + 2), str_value, 10);
			device_MicroMario_log("APPN_Power: value = %s, purifier_status.pwr = %s", str_value, purifier_status.pwr_str);
		...
		case APPN_MicroMario_ChildLock:
		{
			b_value = (*(p + 2) == 1) ? true : false;
			device_MicroMario_log("APPN_ChildLock: value = %d, purifier_status.cl = %d", b_value, purifier_status.cl);
		...
		case APPN_MicroMario_AQILight:
		{
			i_value = *(p + 2);
			device_MicroMario_log("APPN_AQILight: value = %d, purifier_status.aqil = %d", i_value, purifier_status.aqil);
		...
		case APPN_MicroMario_UILight:
		{
			...
			if ( str_value[0] == '\0' )
				break;
			device_MicroMario_log("APPN_UILight: value = %s, purifier_status.uil = %s", str_value, purifier_status.uil_str);
		...
		case APPN_MicroMario_DeviceTimer:
		{
			i_value = *(p + 2);
			device_MicroMario_log("APPN_DeviceTimer: value = %d, purifier_status.dt = %d", i_value, purifier_status.dt);
		...
		case APPN_MicroMario_DeviceTimerRemaining:
		{
			i_value = (*(p + 2) << 8) + *(p + 3);
			device_MicroMario_log("APPN_DeviceTimerRemaining: value = %d, purifier_status.dtrs = %d", i_value, purifier_status.dtrs);
		...
		case APPN_MicroMario_OperationMode:
		{
			switch ( *(p + 2) )
			{
			...
			if ( str_value[0] == '\0' )
				break;
			device_MicroMario_log("APPN_Mode: value = %s, purifier_status.mode = %s", str_value, purifier_status.mode_str);
		...
		case APPN_MicroMario_Function:
		{
			...
			device_MicroMario_log("APPN_MicroMario_Function: value = %s, purifier_status.func = %s", str_value, purifier_status.func);
		...
		case APPN_MicroMario_RHSetting:
		{
			i_value = *(p + 2);
			device_MicroMario_log("APPN_MicroMario_RHSetting: value = %d, purifier_status.rhset = %d", i_value, purifier_status.rhset);
		...
		case APPN_MicroMario_RH:
		{
			i_value = *(p + 2);
			device_MicroMario_log("APPN_MicroMario_RH: value = %d, purifier_status.rh = %d", i_value, purifier_status.rh);
		...
		case APPN_MicroMario_Temperature:
		{
			i_value = (int8_t)(*(p + 2));
			device_MicroMario_log("APPN_MicroMario_Temperature: value = %d, purifier_status.temp = %d", i_value, purifier_status.temp);
		...
		case APPN_MicroMario_AQIt:
		{
			purifier_status.air_prop_map |= PHILIPS_AIR_PROP_MAP_AQIT;
			break;
		}
		case APPN_MicroMario_DeviceDisplayParameter:
		{
			purifier_status.air_prop_map |= PHILIPS_AIR_PROP_MAP_DDP;
			itoa(*(p + 2), str_value, 10);
			device_MicroMario_log("APPN_DeviceDisplayParameter: value = %s, purifier_status.ddp = %s", str_value, purifier_status.ddp_str);
		...
		case APPN_MicroMario_RealtimeDeviceDisplayParameter:
		{
			purifier_status.air_prop_map |= PHILIPS_AIR_PROP_MAP_RDDP;
			itoa(*(p + 2), str_value, 10);
			device_MicroMario_log("APPN_RealtimeDeviceDisplayParameter: value = %s, purifier_status.rddp = %s", str_value, purifier_status.rddp_str);
		...
		case APPN_MicroMario_ErrorCode:
		{
			...
			if ( (deviceerrorcode >> 14) & 0x01 )	// warning
			{
				for ( int i = 0; i <= 8; ++i )
				{
					if ( (deviceerrorcode >> i) & 0x01 )
					{
						...
						case 1: get_philips_running_status()->device_upload_manage.warn |= PHILIPS_WARNING_F5; break;	// Wick cleaning
			...
			else if ( (deviceerrorcode >> 15) & 0x01 )	// error
			{
				...
				case 0x8006: get_philips_running_status()->device_upload_manage.error |= PHILIPS_ERROR_E6; break;	// Gear motor error
				default: break;
				}
			}

			device_MicroMario_log("purifier_status.err = %d, deviceerrorcode = %lu, warn = 0x%08X, error = 0x%08X", purifier_status.err, deviceerrorcode, get_philips_running_status()->device_upload_manage.warn, get_philips_running_status()->device_upload_manage.error);
			device_MicroMario_log("...");
		...
		case APPN_MicroMario_WaterLevel:
		{
			i_value = *(p + 2);
			device_MicroMario_log("APPN_MicroMario_WaterLevel: value = %d, purifier_status.wl = %d", i_value, purifier_status.wl);
