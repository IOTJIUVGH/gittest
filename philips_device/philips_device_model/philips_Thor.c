...
//#define device_Thor_log(M, ...)				custom_log("", M, ##__VA_ARGS__)
#define device_Thor_log(M, ...)

// Thor
enum _Air_properties_property_no_Thor
{
	APPN_Thor_OpMode								= 0x01,
	APPN_Thor_Power									= 0x02,
	APPN_Thor_ChildLock								= 0x03,
	APPN_Thor_AQILight								= 0x04,
	APPN_Thor_UILight								= 0x05,
	APPN_Thor_Mode									= 0x06,
	APPN_Thor_PM2_5									= 0x07,
	APPN_Thor_iAQL									= 0x08,
	APPN_Thor_AQIt									= 0x09,
	APPN_Thor_TVOC									= 0x0A,
	APPN_Thor_DeviceDisplayParameter				= 0x0B,
	APPN_Thor_RealtimeDeviceDisplayParameter		= 0x0C,
	APPN_Thor_ErrorCode								= 0x0D,
};
...
void philips_encode_json_Thor(char **buf, const char *StatusType, const char *ConnectType, bool head)
{
	...
	cJSON_AddStringToObject(item, "range", "Thor");
...
void philips_process_air_properties_Thor(const uint8_t *data)
{
    ...
    while (*(p) != 0x00)
    {
		...
		case APPN_Thor_OpMode:
		{
			...
			if ( str_value[0] == '\0' )
				break;
			device_Thor_log("APPN_FanSpeed: value = %s, purifier_status.om = %s", str_value, purifier_status.om_str);
		...
		case APPN_Thor_Power:
		{
			itoa(*(p + 2), str_value, 10);
			device_Thor_log("APPN_Power: value = %s, purifier_status.pwr = %s", str_value, purifier_status.pwr_str);
		...
		case APPN_Thor_ChildLock:
		{
			b_value = (*(p + 2) == 1) ? true : false;
			device_Thor_log("APPN_ChildLock: value = %d, purifier_status.cl = %d", b_value, purifier_status.cl);
		...
		case APPN_Thor_AQILight:
		{
			itoa(*(p + 2), str_value, 10);
			device_Thor_log("APPN_AQILight: value = %s, purifier_status.aqil = %s", str_value, purifier_status.aqil_str);
		...
		case APPN_Thor_UILight:
		{
			itoa(*(p + 2), str_value, 10);
			device_Thor_log("APPN_UILight: value = %s, purifier_status.uil = %s", str_value, purifier_status.uil_str);
		...
		case APPN_Thor_Mode:
		{
			...
			if ( str_value[0] == '\0' )
				break;
			device_Thor_log("APPN_Mode: value = %s, purifier_status.mode = %s", str_value, purifier_status.mode_str);
		...
		case APPN_Thor_DeviceDisplayParameter:
		{
			purifier_status.air_prop_map |= PHILIPS_AIR_PROP_MAP_DDP;
			itoa(*(p + 2), str_value, 10);
			device_Thor_log("APPN_DeviceDisplayParameter: value = %s, purifier_status.ddp = %s", str_value, purifier_status.ddp_str);
		...
		case APPN_Thor_RealtimeDeviceDisplayParameter:
		{
			purifier_status.air_prop_map |= PHILIPS_AIR_PROP_MAP_RDDP;
			itoa(*(p + 2), str_value, 10);
			device_Thor_log("APPN_RealtimeDeviceDisplayParameter: value = %s, purifier_status.rddp = %s", str_value, purifier_status.rddp_str);
		...
		case APPN_Thor_ErrorCode:
		{
			...
			device_Thor_log("purifier_status.err = %d, deviceerrorcode = %lu, warn = 0x%08X, error = 0x%08X", purifier_status.err, deviceerrorcode, get_philips_running_status()->device_upload_manage.warn, get_philips_running_status()->device_upload_manage.error);
			device_Thor_log("...");
