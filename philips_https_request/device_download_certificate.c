...
static merr_t https_response_data_process(char *data, int len, char *head, int head_len)
{
	...
	if(err == kNoErr && (value_len == 32 || value_len == 34))
	{
		app_log("MD5:%.*s\r\n", value_len, value);
	...
	if ( (get_philips_running_status()->flash_param.certificate_len != len) || (memcmp(get_philips_running_status()->certificate, data, len) != 0) )
	{
		app_log("update certificate");

		// 擦除证书分区
		mhal_flash_erase(MODULE_PARTITION_USER, CERTIFICATE_OFFSET, 2048);

		// 证书写入flash
		memset(get_philips_running_status()->certificate, 0x00, sizeof(get_philips_running_status()->certificate));
		memcpy(get_philips_running_status()->certificate, data, len);
		err = mhal_flash_write(MODULE_PARTITION_USER, &offset, (uint8_t *)(get_philips_running_status()->certificate), (uint32_t)len);
		require_noerr_action(err, exit, err = kWriteErr);

		// 证书从flash重新读取
		offset = CERTIFICATE_OFFSET;
		memset(get_philips_running_status()->certificate, 0x00, sizeof(get_philips_running_status()->certificate));
		err = mhal_flash_read(MODULE_PARTITION_USER, &offset, (uint8_t *)(get_philips_running_status()->certificate), (uint32_t)len);
		require_noerr_action(err, exit, err = kReadErr);

		// 证书MD5计算
		InitMd5(&ctx);
		Md5Update(&ctx, (uint8_t *)(get_philips_running_status()->certificate), len);
		...
		{
			app_log("certificate md5 verify fail, recv_md5:%s, calc_md5:%s", md5_recv_str, md5_calc_str);
			err = kChecksumErr;
			goto exit;
		}
		else
		{
			app_log("certificate md5 verify success");
		}

		get_philips_running_status()->flash_param.certificate_len = len;
		mkv_item_set("pmifhr_clen", &get_philips_running_status()->flash_param.certificate_len, sizeof(get_philips_running_status()->flash_param.certificate_len));
	}
	else
	{
		app_log("certificate don't change");
	}
...
static void https_request_thread(void *arg)
{
	...
	err = device_http_param_init(&req, get_philips_running_status()->certificate_url, &body, max_body_len, max_req_len);
	...
	req.req_len = snprintf(req.http_req, max_req_len, HTTP_COMMON_GET, get_philips_running_status()->certificate_url, req.domain_name, user_agent);
	...
	{
		app_log("download_certificate error, enter block mode");

		if (get_philips_running_status()->is_first_config_wifi && !(get_philips_running_status()->softap_option & 0x08))
		{
			get_philips_running_status()->wifi_log_param.step = PHILIPS_WIFI_LOG_STEP_DOWNLOAD_CERTIFICATION;
...
merr_t philips_device_download_certificate(void)
{
	...
	thread = mos_thread_new(MOS_APPLICATION_PRIORITY, "download_certificate", https_request_thread, 0x1000, NULL);
